...
def response(flow: mitmproxy.http.HTTPFlow):
    ...
    if flow.server_conn and flow.server_conn not in SERVERS_SEEN:
        if flow.server_conn.timestamp_tcp_setup is not None:
    ...
    if flow.server_conn.connected:
        if flow.server_conn.peername is not None and len(flow.server_conn.peername) > 0:
            entry["serverIPAddress"] = str(flow.server_conn.peername[0])
        else:
            entry["serverIPAddress"] = "0.0.0.0"
...
def dump_har():
    print("ctx.options.hardump", ctx.options.hardump)
