...
def test_location(flow: http.HTTPFlow, spec: UrlRedirectSpec) -> bool:
    if spec.location["scheme"]:
        if not fnmatch.fnmatch(flow.request.scheme, spec.location["scheme"]):
            # logging.info("Scheme not match: " + flow.request.scheme + spec.location["scheme"])
            return False

    if spec.location["host"]:
        if not fnmatch.fnmatch(flow.request.pretty_host, spec.location["host"]):
            # logging.info("host not match: " + flow.request.pretty_host + spec.location["host"])
            return False

    if spec.location["port"]:
        port = int(spec.location["port"])
        if flow.request.port != port:
            # logging.info("port not match: " + flow.request.port + spec.location["port"])
            return False

    if spec.location["path"]:
        if not fnmatch.fnmatch(flow.request.path, spec.location["path"]):
            # logging.info("path not match: " + flow.request.path + spec.location["path"])
            return False

    # logging.info(where + " Location Matched")
    return True


def test_rule_match(flow: http.HTTPFlow, rule, where):
    if not rule["enabled"]:
        # logging.info("Rule Not enabled")
        return False

    if len(rule["where"]) > 0:
        if (where not in rule["where"]):
            # logging.info("reqeust / response not matched")
            return False
    ...
    if (rule["type"] in [
        ...
        if where == 'response':
            # logging.info(rule["type"] + " didn't work in response")
            return False
    ...
    if (rule["type"] in [
        ...
        if where == 'request':
            # logging.info(rule["type"] + " didn't work in request")
            return False
    ...
    # ].index(rule["type"]) > -1:
    #     return False
    # logging.info(where + " Rule Matched")
...
def response_replacement(flow: http.HTTPFlow, rule):
    logging.info(rule["replace"]["name"])
    logging.info(rule["replace"]["value"])
    logging.info(flow.response.headers)
    ...
    if rule["type"] == 'remove_header':
        if hasattr(flow.response.headers, rule["match"]["name"]):
            # logging.info(hasattr(flow.response.headers, rule["match"]["name"]))
            del flow.response.headers[rule["match"]["name"]]
    ...
    if rule["type"] == 'body':
        ...
        # logging.info(flow.response.encoding)
        # logging.info(flow.response.content)
        # logging.info("text " + flow.request.get_text())
    ...
    def configure(self, updated):
        if "rewrite" in updated:
            ...
            for option in options:
                logging.info("[Rewrite] " + option["description"])

                try:
                    spec = parse_rewrite_spec(option=option)
                except ValueError as e:
                    logging.info(f"Cannot parse rewrite option {option}: {e}")
                    # raise exceptions.OptionsError(f"Cannot parse rewrite option {option}: {e}") from e
                    continue
                except Exception as e:
                    logging.info(f"Exception {option}: {e}")
